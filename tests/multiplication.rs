...
fn degree32_mul() {
    ...
    let mut p1 = Polynomial::zeros( 32 );
    let mut p2 = Polynomial::zeros( 32 );

    for i in 0 .. 20 {
        println!("Test {}", i );
        for i in 0 .. 32 {
            p1[i] = rng.gen_range( -30, 30 );
            p2[i] = rng.gen_range( -30, 30 );
        }
        println!("p1 = {}", p1 );
        println!("p2 = {}", p2 );

        let plan_maybe = KaratsubaPlan::<i32>::new_plan( 32 );

        let mut ka = Polynomial::with_capacity( 48 );
